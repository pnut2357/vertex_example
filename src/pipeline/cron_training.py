...
# Network, GCP Project id, service account, region, and docker images. 
...
def element_model_registry(
    ...
    # Parameters
    version = int(c.get_latest_versions(model_registry_name, stages=["None"])[0].version) + 1
    ...
    # except:
    #     # Create experiment 
    #     experiment_id = c.create_experiment(mlflow_exp_name, artifact_location=artifact_repository)
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name=run_name) as run:
        ...
        tags = {"Application": "Order Your Inventory",
                "tags_model_version": f"{str(version)}",
        ...
        # log metrics
        mlflow.log_metrics({"stack_auc": current_auc_score})
        
    element_model_registry_output = namedtuple("Outputs", ["model_registry_name", "mlflow_exp_name", "current_version", "current_auc_score"])
    return element_model_registry_output(model_registry_name, mlflow_exp_name, version, current_auc_score)
...
def version_transition(
    ...
    previous_metrics, previous_model_version = mlflow_obj.find_metrics(model_registry_name)
    
    if current_auc_score >= previous_metrics["stack_auc"]:
        # Transition the latest version to Staging
        mlflow_obj.model_version_registry(model_registry_name, current_version, "Staging")
        # Transition the latest version to Production
        mlflow_obj.model_version_registry(model_registry_name, current_version, "Production")
...
def pipeline():    
    ...
    with Condition(train_eval_data.outputs["Output"] >= 0.7, name='model-upload-condition'):
        element_model_registry_job = element_model_registry(nosales_model_input=train_eval_data.outputs['nosales_model_output'],
                                                            model_registry_name=MODEL_REGISTRY_NAME,
                                                            mlflow_exp_name=MLFLOW_EXP_NAME,
                                                            current_auc_score=train_eval_data.outputs["Output"])
        
        version_transition_job = version_transition(model_registry_name=element_model_registry_job.outputs["model_registry_name"], 
                                                mlflow_exp_name=element_model_registry_job.outputs["mlflow_exp_name"], 
                                                current_version=element_model_registry_job.outputs["current_version"],
                                                current_auc_score=element_model_registry_job.outputs["current_auc_score"])
